...
import type { Story, StoryPost } from '@/types/story';
import { updateStory } from '@/lib/api/clientApi';
...
export default function EditStoryForm({ story }: EditStoryFormProps) {
  ...
  const { draft, setDraft } = useStoryDraftStore();
  ...
  const initialValues: StoryFormValues = {
    ...
    img: story?.img as unknown as string,
  };
  ...
  const { mutate: updateStoryMutation, isPending } = useMutation({
    mutationFn: (payload: Partial<StoryPost>) => updateStory(story.id, payload),
    onSuccess() {
      queryClient.invalidateQueries({ queryKey: ['stories'] });
      queryClient.invalidateQueries({ queryKey: ['story', story.id] });
      toast.success('Історія успішно оновлена');
    },
    onError() {
      toast.error('Помилка при оновленні історії');
    },
  });
  ...
  const onSubmit = (values: StoryFormValues) => {
    updateStoryMutation({
      ...
      ...(values.article ? { article: values.article } : {}),
      ...(values.category ? { category: values.category } : {}),
      ...(coverImage?.file ? { img: coverImage.file } : {}),
    });
  };
  ...
  return (
    <Formik
      ...
      onSubmit={onSubmit}
    >
      {({ values, handleChange, setFieldValue, errors, touched, isValid, dirty, resetForm }) => {
        return (
          <>
